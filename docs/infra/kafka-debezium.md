...
이 문서는  
`Producer(Service) -> Outbox(DB Table) -> Debezium(CDC) -> Kafka -> Consumer(Service)`  
흐름을 로컬 개발과 운영 관점에서 정리한 문서다.
...
```mermaid
flowchart LR
    A["각 서비스"] --> B["도메인 테이블 + outbox"]
    B --> C["MySQL Binlog"]
    C --> D["Debezium Connector"]
    D --> E["Kafka Topic\noutbox.event.{EventType}"]

    E --> F["다른 서비스 비즈니스 컨슈머"]
    E --> G["Outbox Status Consumer\n(self-consume)"]
...
- 서비스별 connector 설정: `{svc}/infra/debezium/{svc}-outbox-connector.json`
    - 예시: [order-outbox-connector-smt.json](../../Order/infra/debezium/order-outbox-connector-smt.json)

## 4. connector 등록 방식

현재 프로젝트는 Debezium connector 등록을 `POST /connectors`가 아니라 `PUT /connectors/{name}/config` 방식으로 맞췄다.

정리:

- 없으면 생성, 있으면 같은 이름으로 설정 갱신 (Upsert 방식)

중요한 포인트:

- `PUT` body는 `name + config` 전체 JSON이 아니다.
- 각 서비스 JSON 안의 `name`은 URL에 쓰고,
- 실제 request body는 JSON 내부 `config` 내용물만 보내야 한다.

예시:

```cmd
curl -X PUT http://localhost:8083/connectors/order-outbox-connector/config -H "Content-Type: application/json" --data-binary "@order-outbox-connector-config.json"
```

여기서 `order-outbox-connector-config.json`은 설명용 예시 파일명이다.
실제 서비스 JSON은 보통 `name + config` 전체를 담고 있고, 등록 스크립트가 그 안에서 `config`만 추출해서 PUT body로 보낸다.

실제 등록 스크립트는 아래 파일을 사용한다.

- [infra/debezium/register-all-outbox-connectors.cmd](../../infra/debezium/register-all-outbox-connectors.cmd)

## 5. 토픽

토픽은 event_type 기반으로 간다.
...
aggregate 기반에서 event_type 기반 토픽으로 바꾼 이유는 아래와 같다.
- 예전에는 `aggregate_type` 기준으로 `outbox.event.ORDER`, `outbox.event.USER` 같은 큰 토픽으로 묶었다.
- 이 방식은 토픽 수는 적지만, 한 aggregate 안의 서로 다른 이벤트가 한 토픽에 같이 들어와서 consumer가 불필요한 메시지까지 받아서 필터링 처리를 해야했다.
  - 예를 들면, `Payment` 서비스의 Consumer 에서 실제로 필요한 건 `PaymentRequested`하나뿐이지만, `ORDER` 토픽을 구독하면 `OrderCreated`, `OrderValidated`, `OrderStatusChanged` 같은 이벤트까지 들어옴.
- 그래서 현재는 Debezium Outbox SMT의 `route.by.field`를 `aggregate_type`에서 `event_type`으로 바꿔, 이벤트 의미 단위로 토픽을 분리했다.
- 그 결과 consumer는 필요한 이벤트 토픽만 직접 구독하면 되고, eventType 필터링 부담도 줄었다.

레거시 aggregate 토픽 (SMT 미적용 시 발행되는 raw aggregate 토픽, 호환/디버깅용):
...
- `connect`는 `REST_ADVERTISED_HOST_NAME=connect`
  - Connect가 내부 네트워크에서 자기 자신을 광고할 때 쓰는 호스트명이다. 이 값을 고정하지 않으면 컨테이너 ID 같은 값으로 잡혀서 다른 컴포넌트에서 REST endpoint를 못 찾는 경우가 있다.
...
3. 상태를 `INIT`으로 되돌린다
4. 현재 `event_type`에 맞는 SMT 토픽으로 payload를 다시 발행한다
   - 예: `PaymentRequested -> outbox.event.PaymentRequested`
...
- 기본 source topic은 서비스별 CDC 원본 토픽을 사용한다
  - 예: `delivery.delivery_order.outbox`
  - 예: `delivery.delivery_auth.outbox`
- 주 소비 토픽은 `event_type` 기반 SMT 토픽을 사용한다
  - 예: `outbox.event.OrderCreated`
  - 예: `outbox.event.PaymentRequested`
- 과거에는 source topic과 aggregate 기반 SMT topic을 함께 구독하는 consumer도 있었지만, 현재 주 소비 기준은 event_type 기반 SMT 토픽이다

참고:

- 현재 converter 영향으로 SMT 토픽 메시지 값에 `schema` / `payload` 래퍼가 붙을 수 있다
- 일부 consumer는 header / envelope / SMT wrapper를 모두 처리하도록 작성돼 있다
...
위 방식 상세 설명은 문서 상단 `connector 등록 방식` 섹션 기준으로 본다.

## 13. 장애 확인 순서

### 13.1 주문은 생성됐는데 Kafka 메시지가 안 보일 때
...
### 13.2 Kafka 메시지는 있는데 Outbox가 계속 INIT일 때
...
### 13.3 Connect 로그에 `Node disconnected`가 반복될 때
...
- SMT 토픽 `outbox.event.{EventType}` 기준으로 다음 서비스가 소비한다
...
즉 이 프로젝트에서 Kafka / Debezium의 역할은 “각 서비스의 도메인 상태 변경과 이벤트 발행을 느슨하게 연결하면서도 정합성을 잃지 않게 만드는 것”이다.
